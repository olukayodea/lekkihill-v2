<?php
class labouratory_component extends common {
    public $id;
    public $minify = false;
    public $filter = null;
    public $search = null;
    ...
    public function create($array) {
        ...
        $id = $this->replace(table_name_prefix."labouratory_component", $array, $replace);
    ...
    function modifyOne($tag, $value, $id) {
        if ($this->updateOne(table_name_prefix."labouratory_component", $tag, $value, $id, "ref", "`modify_time` = ".time())) {
            return true;
        } else {
            return false;
        }
    ...
    public function getCount() {
        $query = "SELECT COUNT(`ref`) FROM ".table_name_prefix."labouratory_component WHERE `status` = 'ACTIVE'";
        return $this->query($query, false, "getCol");
    }

    public function getList($start=false, $limit=false, $order="title", $dir="ASC", $type="list") {
        return $this->lists(table_name_prefix."labouratory_component", $start, $limit, $order, $dir, "`status` != 'DELETED'", $type);
    }

    public function getSingle($val, $tag="title", $ref="ref") {
        return $this->getOneField(table_name_prefix."labouratory_component", $val, $ref, $tag);
    }

    public function listOne($id) {
        return $this->getOne(table_name_prefix."labouratory_component", $id, "ref");
    }

    public function getActive() {
        return $this->query("SELECT * FROM `".table_prefix.table_name_prefix."labouratory_component` WHERE `status` = 'ACTIVE' ORDER BY `title` ASC", false, "list");
    }
    ...
    public function getSortedList($id, $tag, $tag2 = false, $id2 = false, $tag3 = false, $id3 = false, $order = 'title', $dir = "ASC", $logic = "AND", $start = false, $limit = false) {
        return $this->sortAll(table_name_prefix."labouratory_component", $id, $tag, $tag2, $id2, $tag3, $id3, $order, $dir, $logic, $start, $limit);
    }

    private function listPages($start, $limit) {
        $return['data'] = $this->lists(table_name_prefix."labouratory_component", $start, $limit, "title", "ASC", "`status` != 'DELETED'");
        $return['counts'] = $this->lists(table_name_prefix."labouratory_component",  false, false, "title", "ASC", "`status` != 'DELETED'", "count");
    ...
    private function runSearch($search, $start, $limit, $type="list") {
        ...
        return $this->query("SELECT * FROM `".table_prefix.table_name_prefix."labouratory_component` WHERE (`title` LIKE :search OR `cost` LIKE :search OR `description` LIKE :search OR `status` LIKE :search) AND `status` != 'DELETED' ORDER BY `title` ASC".$add, array(':search' => "%".$search."%"), $type);
